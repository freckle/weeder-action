...
const ARTIFACT_PREFIX = 'https://github.com/freckle/weeder-action/releases/download/Binaries'

async function installWeeder(ghcVersion: string): Promise<string> {
  const {platform: os, arch} = process
  const [ext, extract] = os === 'win32' ? [`zip`, tc.extractZip] : [`tar.gz`, tc.extractTar]
